...
export class CamposController {
  ...
  async jobs(): Promise<string> {
    return await this.camposService.executeKw('hr.job', 'search_read', [], {
      fields: ['name', 'organization_id', 'state', 'user_id', 'description', 'description_time_and_scope', 'description_we_give', 'description_you_give', 'no_of_recruitment', 'no_of_hired_employee', 'write_date', 'create_date'],
